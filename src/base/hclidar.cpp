...
bool HCLidar::setLidarPara(const char* chLidarModel)
{
    ...
    else if (m_strLidarModel == T200){
        m_sAttr.dAngleOffsetD = 0;
        m_sAttr.dBaseline_mm = 0;
        m_sAttr.dTheta_d = 0;
        m_sAttr.iFPSMax = FPS_5000_MAX;
        m_sAttr.iFPSMin = FPS_5000_MIN;
        m_sAttr.iSpeedMax = SPEED_360_MAX;
        m_sAttr.iSpeedMin = SPEED_360_MIN;
        m_sAttr.dAngleStep = ANGLE_RESOLVE_5000_6HZ;
        m_sAttr.dCirclePoints = CIRCLE_MAX_5000_6HZ;
        m_sAttr.bAngOffset = false;
        m_sAttr.bBlockSpeed = false;

        m_sAttr.iFPSNor = FPS_5000_NOR;
        m_sAttr.iSpeedNor = SPEED_360_NOR;
    }
    else
    {
...
void HCLidar::threadWork()
{
    while (m_bScanning)
    {
        /// T2 condition variable checked
        if(m_strLidarModel == T200 && !m_bT2Stared){
            std::unique_lock<std::mutex> ul(m_mtxT2Stared);
            m_cvT2Started.wait(ul, [=](){
                return m_bT2Stared;
            });
        }
        readData();
...
void HCLidar::readData()
{
    ...
    static unsigned char receivedLastFiveBytes[5];
			...
			if (m_u64StartTimeNoData > 0)
			{
				...
				if (endTime >= (m_u64StartTimeNoData+m_sSDKPara.iNoDataMS * 1000))
				{
					...
					if (m_iReadTimeoutCount >= iTemp2){
                        /// add locked-rotor for T2
                        bool bLockedRotor {false};
                        if (m_strLidarModel == T200){
//                            std::cout << "receivedLastFiveByte: ";
//                            for (unsigned char receivedLastFiveByte : receivedLastFiveBytes) {
//                                std::cout << std::hex << static_cast<int>(receivedLastFiveByte) << " ";
//                            }
//                            std::cout << std::endl;

                            bLockedRotor = true;
                            constexpr unsigned char uLockedRotor[5]{0xA5, 0xAA, 0x5A, 0x54, 0x02};
                            for (int i = 0; i < 5; ++i) {
                                if (uLockedRotor[i] != receivedLastFiveBytes[i]){
                                    bLockedRotor = false;
                                    break;
                                }
                            }
    ...
    const int iSize = sizeof(receivedLastFiveBytes);
    if (iRC >= iSize){
        memcpy(receivedLastFiveBytes, m_p8Buff + iRC - iSize, iSize);
    }
...
void HCLidar::threadParse()
{
    ...
    while (m_bScanning)
    {
        ...
        if(m_strLidarModel == T200 && !m_bT2Stared){
            std::unique_lock<std::mutex> ul(m_mtxT2Stared);
            m_cvT2Started.wait(ul, [=, &u64Start, &u64LastSend](){
                m_u64TSEnd = HCHead::getCurrentTimestampNs();
                u64Start = HCHead::getCurrentTimestampUs();
                // make sure first call
                u64LastSend = u64Start - sendTimeIntervalMs;
                return m_bT2Stared;
            });
        }
		...
		if (!m_bInitTimeout)
		{
            ...
            UINT64 u64IntervalSendCommand = (u64End - u64LastSend) / 1000;
			...
			if (u64End >= (u64Start + m_iWaitIDTimeMs* 1000))
			{
                ...
                /// send get T2 lidar info command
                if (m_strLidarModel == T200
                && !m_bHadID
                && u64IntervalSendCommand > sendTimeIntervalMs){
                    sendGetT2LidarInfoCommand();
                    u64LastSend = u64End;
                }
            }
		}
...
//    std::cout << std::hex << "sum_check: " << sum_check << std::endl;
//    std::cout << std::hex << "cal_sum: " << sum << std::endl;
    return (sum_check == sum);
}
...
bool HCLidar::parserRangeEX(LstPointCloud &resultRange, const char * chBuff, int iIndex, int in_numData, int iPointSize)
{
    ...
    if (m_strLidarModel == T200){
        FA = (((uint8_t)chBuff[id_start + 1] << 8) + (uint8_t)chBuff[id_start]) / 100.0;
        LA = (((uint8_t)chBuff[id_LA_start + 1] << 8) + (uint8_t)chBuff[id_LA_start]) / 100.0;
        speed = ((((uint8_t)chBuff[id_start - 1] << 8) + (uint8_t)chBuff[id_start - 2]) / 64);
    }
	...
	for (int i = 0; i < in_numData; ++i)
	{
		...
		if (m_strLidarModel == "T3B" || m_strLidarModel == T200)
...
bool HCLidar::getT2SNInfo(std::vector<UCHAR> &lstBuff) {
    LOG_INFO("getT2SNInfo!\n");

    if (lstBuff.size() < sizeof(tsSDKSNT200))
        return false;

    tsSDKSNT200 sSdkSnT200;
    const int iT2Size = sizeof(tsSDKSNT200);
    memcpy(&sSdkSnT200, lstBuff.data(), iT2Size);

    if (calStartInfo((char*)lstBuff.data(), iT2Size)){
        HCHead::eraseBuff(lstBuff, iT2Size);

        char chTemp[128] = { 0 };
        sprintf(chTemp, "%c%c", sSdkSnT200.u8FacInfo[0], sSdkSnT200.u8FacInfo[1]);
        m_strFactoryInfo = chTemp;
        memset(chTemp, 0, 128);

        sprintf(chTemp, "%c%d%c%c",
                sSdkSnT200.u8ProType[0], sSdkSnT200.u8ProType[1],
                sSdkSnT200.u8ProType[2], sSdkSnT200.u8ProType[3]);
        std::string strModel = chTemp;
        if (strModel != m_strLidarModel)
        {
            LOG_WARNING("Lidar model error init:%s,device:%s\n",
        ...
        sprintf(chTemp, "%02X.%02X.%02X.%02X",
                sSdkSnT200.u8FirmwareVer[0],
                sSdkSnT200.u8FirmwareVer[1],
                sSdkSnT200.u8FirmwareVer[2],
                sSdkSnT200.u8FirmwareVer[3]);
        m_strFirmwareVer = chTemp;
        memset(chTemp, 0, 128);

        sprintf(chTemp, "%02X.%02X.%02X.%02X",
                sSdkSnT200.u8HardwareVer[0],
                sSdkSnT200.u8HardwareVer[1],
                sSdkSnT200.u8HardwareVer[2],
                sSdkSnT200.u8HardwareVer[3]);
        m_strHardwareVer = chTemp;
        memset(chTemp, 0, 128);
        ...
        sprintf(chTemp, "%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c",
                ...
                sSdkSnT200.u8SN[19] );
        LOG_INFO("Get SN:%s\n", chTemp);
        // Remove reserved invalid characters
        const int invalidCharPosition = 15;
        const int iT2SnLength = 32;
        for (int i = invalidCharPosition; i < iT2SnLength; ++i) {
            chTemp[i] = '0';
        }
        chTemp[iT2SnLength] = '\0';
        m_strDevID = chTemp;
        memset(chTemp, 0, 128);
        ...
        HCHead::eraseBuff(lstBuff, iT2Size);
        LOG_ERROR("T2 lidar factory info cal error\n");
        setReadCharsError(ERR_START_INFO);
        return true;
    }
...
void HCLidar::setT2RotationalSpeed(int iRotationalSpeed) {
    ...
    UCHAR command[iLen]{0xAA, 0x55, 0x07, 0x2A};
    command[4] = uint8_t(iRotationalSpeed % 256); // lower
    command[5] = uint8_t(iRotationalSpeed / 256); // upper
    for (int i = 2; i < iLen - 1; ++i) {
        paraChecksum += command[i];
    }
    command[6] = paraChecksum;

    /// write the control speed code
    int writeLen {0};
    ...
    if (writeLen != iLen){
        ...
        LOG_INFO("Send change speed succeed! \n");
        m_sAttr.iSpeedNor = iRotationalSpeed;
        m_sAttr.iSpeedMax = iRotationalSpeed + 10;
        m_sAttr.iSpeedMin = iRotationalSpeed - 10;
        double dRoundPerSecond = iRotationalSpeed / 60.0;
        const int iT2FPS = FPS_5000_NOR;
...
void HCLidar::sendGetT2LidarInfoCommand() {
    if (m_strLidarModel != T200){
        LOG_WARNING("Only support T2");
        return;
    }
    ...
    if (writeLen != iLen){
        LOG_ERROR("Send get T2 lidar info failed! \n");
    }
    else {
        LOG_INFO("Send get T2 lidar info succeed! \n");
    }
}

void HCLidar::setT2LidarStart(bool bStarted) {
    if (m_strLidarModel != T200){
        LOG_WARNING("Only support T2");
        return;
    }
    ...
    if (m_bT2Stared){
        ...
        if (writeLen != iLen){
            ...
            LOG_INFO("Send start T2 lidar succeed! \n");
            setPointCloudLattice(false);
            setAngOffset(false);
        ...
        if (writeLen != iLen){
            LOG_ERROR("Send stop T2 lidar failed! \n");
        }
        else {
